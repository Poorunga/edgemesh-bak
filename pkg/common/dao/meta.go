...
// SaveMeta save meta to dao
func SaveMeta(meta *Meta) error {
	num, err := DBAccess.Insert(meta)
...
func DeleteMetaByKey(key string) error {
	num, err := DBAccess.QueryTable(MetaTableName).Filter("key", key).Delete()
...
func UpdateMeta(meta *Meta) error {
	num, err := DBAccess.Update(meta) // will update all field
...
func InsertOrUpdate(meta *Meta) error {
	_, err := DBAccess.Raw("INSERT OR REPLACE INTO meta (key, type, value) VALUES (?,?,?)", meta.Key, meta.Type, meta.Value).Exec() // will update all field
...
func UpdateMetaField(key string, col string, value interface{}) error {
	num, err := DBAccess.QueryTable(MetaTableName).Filter("key", key).Update(map[string]interface{}{col: value})
	klog.V(4).Infof("Update affected Num: %d, %v", num, err)
	return err
}
...
func UpdateMetaFields(key string, cols map[string]interface{}) error {
	num, err := DBAccess.QueryTable(MetaTableName).Filter("key", key).Update(cols)
	klog.V(4).Infof("Update affected Num: %d, %v", num, err)
	return err
}
